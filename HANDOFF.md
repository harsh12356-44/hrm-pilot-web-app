...
24. **Strict Admin Access Control & Individual Profile Attendance/Working Hours Grids**: Restricted `/admin/*` routes and the `HR Admin Suite` button in [`Navbar.tsx`](file:///d:/Ravina/Antigravity/hrm-pilot-web-app/src/components/Navbar.tsx) and [`Sidebar.tsx`](file:///d:/Ravina/Antigravity/hrm-pilot-web-app/src/components/Sidebar.tsx) strictly to **Ravina Khimani (`RK001`)**. All other Employee and Manager logins (e.g. Naman Bangia, Jigyasa Sen, Sonu Goswami, Anup Sen, etc.) access their own Employee/Manager portals where they see their own personal monthly attendance grid and working hours grid.
